...
public interface OrderDao {
    ...
    @Delete("delete from miaosha_order where user_id = #{userId} and goods_id = #{goodsId} and order_id = #{orderId}")
    void deleteMiaoshaOrder(@Param("userId") long userId, @Param("goodsId") long goodsId, @Param("orderId") long orderId);

    @Select("select * from order_info where status = 0")
    List<OrderInfo> selectExpireOrders();
